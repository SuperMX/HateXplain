...
#         m = nn.Softmax(dim=1)
    ...
    tn0, fp0, fn0, tp0 = confusion_matrix([1-n for n in true_labels], [1-n for n in pred_labels]).ravel()
    confusion_dict_neutral={"TN":tn0,"FP":fp0,"FN":fn0,"TP":tp0}
    tn1, fp1, fn1, tp1 = confusion_matrix(true_labels, pred_labels).ravel()
    confusion_dict_target={"TN":tn1,"FP":fp1,"FN":fn1,"TP":tp1}
    cat_stats={params['target'] : confusion_dict_target, "None" : confusion_dict_neutral}
...
def get_final_dict_with_rational(params,test_data=None,topk=5):
    ...
    #list_dict_with_rational,_=standaloneEval(params, test_data=test_data_with_rational, topk=topk,use_ext_df=True)
    #test_data_without_rational=convert_data(test_data,params,list_dict_org,rational_present=False,topk=topk)
    #list_dict_without_rational,_=standaloneEval(params, test_data=test_data_without_rational, topk=topk,use_ext_df=True)
    final_list_dict=[]
...
if __name__=='__main__': 
    ...
    #test_data=get_test_data(temp_read,params,message='text')
    #standaloneEval(params, params['data_file'],topk=5)
